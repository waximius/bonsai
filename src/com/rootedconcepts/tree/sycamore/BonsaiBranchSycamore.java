 ...
 */
package com.rootedconcepts.tree.sycamore;
 ...
 * @author TheDrydens
 */
public class BonsaiBranchSycamore extends BonsaiBranch {
    ...
    private BonsaiTreeSycamore m_tree = null;
    
    private BonsaiBranchSycamore m_parentBranch = null;
    private BonsaiBranchSycamore m_leftBranch = null;
    private BonsaiBranchSycamore m_rightBranch = null;
    
    public static final int TINY_ANGLE  = 4;
    ...
    public BonsaiBranchSycamore (BonsaiTreeSycamore tree, BonsaiBranchSycamore parentBranch,
    ...
    public void growTrunk(int step, BonsaiBranchInterface prevBranch) {
        ...
        // Otherwise grow the children
            ...
            if (null != m_leftBranch) {
                m_leftBranch.growTrunk(step - 1, this);
            }
        }
    }
     ...
     */
    private float getRadius(BonsaiBranchSycamore branch, int r) {
        final int algo = 2;
        float retval = 0.0f;
        ...
        if (null != branch) {
            if (1 == algo) {
                ...
                retval = 1.0f + (float)(1/-Math.exp(0.03 * (r + (getMaturityLevel() - branch.getMaturityLevel()))));
            } else {
     ...
     * @return A new branch
     */
    private BonsaiBranchSycamore generateBranch (int deltaT, int deltaP, boolean bud) {
        ...
        if (true == M_VARIABLE_BRANCH_LENGTH) {
            if (0.5 > Math.random ()) {
                double length = TREE_SCALE + (Math.random() * (TREE_SCALE * 0.60));
                ...
                double length = TREE_SCALE - (Math.random() * (TREE_SCALE * 0.20));
                x1 = m_x0 - (double) length * Math.sin((double) t1 * Math.PI / 180) * Math.cos((double) p1 * Math.PI / 180);
                y1 = m_y0 - (double) length * Math.sin((double) t1 * Math.PI / 180) * Math.sin((double) p1 * Math.PI / 180);
                z1 = m_z0 + (double) length * Math.cos((double) t1 * Math.PI / 180);
            }
        ...
        return new BonsaiBranchSycamore (m_tree, this, 
            x1, y1, z1, t1, p1,
            stepSize, false, bud
        );
    }
    ...
    private BonsaiBranchSycamoreGeometry m_geometry = null;
    ...
    public Node render (BonsaiApplication app, AssetManager assetManager, float percent, Node rootNode) {
        ...
        if (null == retval) {
            retval = new Node ();
            retval.setName(BonsaiBranchSycamore.TREE_ROOT_NAME);
        }

        Vector3f p0 = getVector3fStart();
        
        BonsaiBranchSycamore[] branches = {m_leftBranch, m_rightBranch};
        for (BonsaiBranchSycamore branch : branches) {
            if (null != branch) {
                ...
                Material treeBarkMat = SycamoreTextureManager.getNewTreeBarkMaterial();
                ...
                Geometry geo = new BonsaiBranchSycamoreGeometry(BonsaiBranchJmeInterface.BRANCH_CYLINDER_NAME, cyl, m_tree, this, branch);
                ...
                branch.setCylinderGeometry((BonsaiBranchSycamoreGeometry)geo);
                ...
                geo = new Geometry(BonsaiBranchSycamore.BRANCH_TIP_NAME_START, sphere);
                ...
                geo = new Geometry(BonsaiBranchSycamore.BRANCH_TIP_NAME_END, sphere);
                ...
                if (true == m_isBud) {
                    ...
                    if ((null == leafCluster.getShape()) || (leafCluster.getShape() instanceof EmitterPointShape)) {
                        EmitterSphereShape emitSphere = new EmitterSphereShape(Vector3f.ZERO, baseRadius * 50);
                        leafCluster.setShape(emitSphere); // TODO make this shaped more upwards than down
                    } else {
                        ((EmitterSphereShape)leafCluster.getShape()).setRadius(baseRadius * 50);
                    }

                    // Size the leaf cluster
                    // TODO make this more heavily exponential
                    leafCluster.setStartSize(baseRadius * 2);
                    leafCluster.setEndSize(baseRadius * 2);

                    // Set our material
                    Material leafMat = SycamoreTextureManager.getLeafMaterial();
                    //Material leafMat = SycamoreTextureManager.getNewLeafMaterial();
     ...
     */
    public void setCylinderGeometry (BonsaiBranchSycamoreGeometry geo) {
        m_geometry = geo;
    }
     ...
     */
    public BonsaiBranchSycamoreGeometry getCylinderGeometry () {
        return m_geometry;
    }
}
